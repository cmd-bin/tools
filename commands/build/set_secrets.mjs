...
function setSecretViaStdin(key, value, cwd) {
  return new Promise((resolve, reject) => {
    const child = spawn("gh", ["secret", "set", key], { cwd, stdio: ["pipe", "ignore", "inherit"] });
...
export async function setSecrets() {
  ...
  if (!hasGh) {
    ...
    if (hasBrew) {
      console.log(pc.cyan("🍺  Homebrew detected. Installing GitHub CLI (gh)..."));
      try {
        execSync("brew install gh", { stdio: "inherit" });
        console.log(pc.green("✅  GitHub CLI (gh) installed successfully!"));
      } catch (err) {
        throw new Error(`Failed to install gh via Homebrew: ${err.message}`);
      }
    } else {
  ...
  if (!fs.existsSync(envPath)) {
    throw new Error(`.env.deploy not found in current directory: ${callerWorkspace}`);
  }
  ...
  if (ignoredKeys.length > 0) {
    console.log(pc.yellow(`⚠️  Ignored keys starting with "GITHUB_" (reserved by GitHub): ${ignoredKeys.join(", ")}`));
  }

  if (keys.length === 0) {
    console.log(pc.yellow("⚠️  No valid environment variables found in .env.deploy."));
    return;
  }

  console.log(pc.cyan(`📤  Uploading ${keys.length} secrets to GitHub repository...`));
  ...
  try {
    ...
    console.log(pc.dim(pc.gray(`(${timeString})`)) + " " + pc.green(`✅  Successfully set the following secrets:`));
    keys.forEach(key => {
      console.log(`   - ${pc.bold(key)}`);
    });
  } catch (err) {
    throw new Error(`Failed to set secrets via gh CLI: ${err.message}`);
  }
}
